...
const AdminDashboard = () => {
  ...
  const getStudentDetails = async () => {
    try {
      const res = await fetch("https://so3h-backend.vercel.app/api/admin/students" , {
        method:"GET"
      });
  ...
  const handleDelete = async (studentId) => {
    ...
    try {
        const res = await fetch(`https://so3h-backend.vercel.app/api/admin/student/delete/${studentId}`, {
        method: "DELETE",
        headers: {
